...
const EMPTY_FORM = {
  ...
  category: '', salary: '', qualification: '', applyLink: '', isNewPost: true,
};
...
export default function JobsList() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(cats);
      if (cats.length > 0 && !form.category && !editJob) {
  ...
  const openAdd = () => { setEditJob(null); setForm(EMPTY_FORM); setError(''); setShowModal(true); };
  const openEdit = (job) => { setEditJob(job); setForm({ ...job }); setError(''); setShowModal(true); };
    ...
    <div>
      ...
      <div className="table-container">
        {loading ? (
          ...
          <table>
            <thead><tr>
              <th>Title</th><th>Organization</th><th>Category</th><th>Last Date</th><th>Status</th><th>Actions</th>
            </tr></thead>
            <tbody>
              {jobs.map((job) => (
                <tr key={job._id}>
              ...
              {jobs.length === 0 && (
                <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--gray)', padding: '3rem' }}>No jobs found. Add your first job!</td></tr>
              )}
            </tbody>
          </table>
        )}
